...
### 5. Application secrets (Key Vault + CSI)
...
**Pattern:** secrets are stored in Key Vault; the **Secrets Store CSI driver** (Azure provider) mounts them into pods as files. Charts may sync to native Kubernetes `Secret` objects where needed.

**Design extension (per-env workload identity):** User-assigned identities `id-boutique-dev/stage/prod` federated to app ServiceAccounts in each namespace, with **Key Vault Secrets User** on the matching vault — see [docs/architecture-design.md §9](docs/architecture-design.md#9-identity-rbac-secrets) for the full design.
