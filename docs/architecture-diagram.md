...
> **Current overview:** The authoritative platform diagram is **[`00-platform-overview.svg`](diagrams/00-platform-overview.svg)** (unified view). The sections below keep the older three-diagram Mermaid style for inline editing and detail; prefer the SVG for an up-to-date single picture.

This doc gives a visual map of the architecture (legacy three Mermaid diagrams from different angles, plus exported SVGs where noted) and a short reference for every tool in the stack with the connections between them.
...
![CI/CD flow](diagrams/01-cicd-flow.svg)

*Rendered files: [SVG](diagrams/01-cicd-flow.svg) · [PNG](diagrams/01-cicd-flow.png) · [DOT source](diagrams/01-cicd-flow.dot)*
...
flowchart LR
  ...
  user["End user"]
  le["Let's Encrypt"]
  dns["Azure DNS<br/>biroltilki.art"]
...
![Azure resource layout](diagrams/02-azure-resources.svg)

*Rendered files: [SVG](diagrams/02-azure-resources.svg) · [PNG](diagrams/02-azure-resources.png) · [DOT source](diagrams/02-azure-resources.dot)*
...
flowchart TB
  ...
  subgraph shared["rg-boutique-shared-weu"]
    ...
    azdns["Azure DNS zone<br/>biroltilki.art"]
...
![Inside the AKS cluster](diagrams/03-inside-cluster.svg)

*Rendered files: [SVG](diagrams/03-inside-cluster.svg) · [PNG](diagrams/03-inside-cluster.png) · [DOT source](diagrams/03-inside-cluster.dot)*
...
flowchart TB
  subgraph external["Outside the cluster"]
    user["End user (HTTPS)"]
    le["Let's Encrypt"]
    azdns["Azure DNS<br/>biroltilki.art"]
...
- **Azure DNS** — Public DNS zone for `biroltilki.art`. Records created automatically by external-dns based on Ingress annotations.
...
| End user resolver | Azure DNS | resolve `*.boutique.biroltilki.art` |
