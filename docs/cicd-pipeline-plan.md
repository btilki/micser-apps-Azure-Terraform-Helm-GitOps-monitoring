...
- **AKS topology**: system + user node pools, autoscaler on, Azure CNI overlay, Workload Identity enabled
...
- Smoke tests after each deploy (curl the frontend, check 200)
...
1. **Stop ingress traffic.** Delete (or de-delegate) DNS records for `*.boutique.biroltilki.art`. Users see clean failures rather than 5xx.
...
10. **Registrar revert.** At your domain registrar, switch `biroltilki.art` NS records away from Azure DNS.
...
4. Fan out to the remaining services
...
**Answer:**I have a real domain name: biroltilki.art
...
**Answer:** Any budget cap: yes or specific node count: yes 
