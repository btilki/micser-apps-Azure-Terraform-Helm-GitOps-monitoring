...
- **`Missing role 'AcrPull' on source ACR`** (for example `acrboutiquedevweu` when running `promote-to-stage`):
  - the app behind **`promotion-azure-connection`** needs **AcrPull** on the **source** registry scope. In this repo, set `promotion_service_principal_object_id` in `terraform.tfvars` for **dev**, **stage**, and **prod** to the **Enterprise application Object ID** (Microsoft Entra admin center → Enterprise applications → **Object ID**; not *Application (client) ID*). Resolve it from DevOps with: `az devops service-endpoint show --id <connection-id> ...` then `az ad sp show --id <serviceprincipalid from JSON> --query id -o tsv`. Then `terraform apply` **dev** (AcrPull on dev ACR), **stage** (AcrPull+AcrPush on stage ACR; Reader on stage and prod RGs), **prod** (AcrPush on prod ACR only—Reader on both RGs is already created by the stage stack to avoid duplicate role assignments). If Azure returns **`PrincipalTypeNotSupported`**, you used an app registration object id or wrong GUID—use the **enterprise application** object id. Alternatively assign roles in the portal or `az role assignment create`.
...
- **`Unable to update target values file`** (Update GitOps values step):
  - a log from **before** the digest job-variable fix may show only that short sentence (no `reason=…` suffix). Re-queue the promotion using **`main`** so the agent runs `promote-image.yml` with **`IMPORTED_IMAGE_DIGEST`** (see merged fix in repo history).
