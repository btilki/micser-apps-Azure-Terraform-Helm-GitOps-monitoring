...
- Terraform destroy fails due to resource lock/dependency:
  - clear workload leftovers in cluster, then rerun `terraform destroy`.
- Remote state access errors during destroy:
  - ensure Blob data-plane role (`Storage Blob Data Contributor`) on tfstate account.
- Namespace stuck in Terminating:
  - clear finalizers as shown above, then retry env stack destroy.
