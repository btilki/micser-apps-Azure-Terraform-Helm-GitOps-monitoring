...
Platform controllers (cert-manager, external-dns) use **Workload Identity**—federated managed identities, no long-lived secrets in manifests. Application secrets use **Key Vault** and the CSI driver for **frontend** (v1; optional Helm mount)—see the repo’s `docs/secrets/key-vault-csi-v1.md`—not plain Git.
