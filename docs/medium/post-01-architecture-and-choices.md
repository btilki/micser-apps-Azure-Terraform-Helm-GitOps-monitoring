# Medium draft — Part 1 of 3

**Suggested title:** Running Google's Online Boutique on Azure: Architecture, GitOps, and Promotion by Digest  
**Subtitle:** Part 1 of 3 — Why one AKS cluster, three registries, and Argo CD beat "kubectl apply" for a serious demo platform  
**Tags (pick 5):** Azure, Kubernetes, DevOps, GitOps, Terraform, AKS, Argo CD, Microservices  
...
2. `01-cicd-flow.png` — in CI/CD section  
3. `02-azure-resources.png` — in Azure foundation section  
...
I wanted a project that felt like real platform engineering—not a single `docker run`, not a slide deck of boxes, and not a cluster where "production" means changing a tag by hand. So I built something that could feel like home to a platform team or an ambitious learner.
...
**Build once, promote the same artifact.** When something reaches stage or prod, it should be the *same image manifest* that passed dev, not a rebuild that might differ because of cache, dependency versions, or networking differences.

**Git as the deployment contract.** The cluster should reconcile from Git (Argo CD), not from "whoever ran helm last." Production changes should need review—and in prod, a human should still approve the sync before it runs.
...
**Isolation without three clusters.** Dev, stage, and prod are separate *logical* environments (namespaces, policies, registries), not three monthly AKS bills for a learning project.
...
A developer pushes application or GitOps changes to GitHub. Azure DevOps builds container images for the services this repo owns, scans them, and pushes to a **development** Azure Container Registry (ACR).

To move toward stage or prod, a **promotion** pipeline runs `az acr import` to copy that digest into the target environment's registry, updates the right values file, and opens another PR. After review and merge, Argo CD sees the new digest and redeploys.
...
That separation—**CI produces artifacts**, **promotion moves artifacts**, **GitOps declares desired state**, **Argo CD reconciles**—is the spine of the design. If you remember one thing from this post, remember this flow.
...
**Per environment** (`dev`, `stage`, `prod`) provisions its own resource group, **its own ACR**, Key Vault, and supporting pieces—so blast radius and permissions can follow environment boundaries exactly.
...
An earlier design used one registry and promoted by retagging. That's simple on paper and messy in practice: tags are mutable, audit trails get fuzzy, and it's too easy for prod to point at an image that's already been overwritten in dev.
...
No rebuild on the way to prod. The digest in `gitops/envs/prod/values-frontend.yaml` (and sibling files) is the contract.
...
There is one AKS cluster. Workloads for the boutique app run in namespaces **`dev`**, **`stage`**, and **`prod`**. Platform components live in their own namespaces: `ingress-nginx`, `cert-manager`, `external-dns`, `kube-prometheus-stack`, and `argocd`.
...
Ingress is HTTPS-only: NGINX Ingress terminates TLS; cert-manager obtains certificates via **DNS-01** against Azure DNS; external-dns keeps records aligned with Ingress hosts. In the reference configuration, only the frontend is public.

Only the **frontend** typically gets a public Ingress. The other owned services (`cartservice`, `currencyservice`, `productcatalogservice`, `redis-cart`) stay `ClusterIP`—which matches how the upstream Google demo is designed.

---

## What this repository actually ships (v1)

To keep the first delivery tractable, the repo **builds and promotes five owned workloads**: frontend, cart, currency, product catalog, and Redis (`redis-cart`). Each has a Helm chart under `charts/` and a CI pipeline.

The remaining Online Boutique services (checkout, payment, email, shipping, recommendations, load generator) can be wired later from **upstream Google images** without this repo rebuilding them—useful for adding complexity in future posts.
...
Pipelines build from `apps/<service>/Dockerfile`, run Trivy (fail on high/critical findings), push to the dev registry, and open a GitHub PR that updates `image.digest` in the dev values file. Triggers are on app changes and scheduled nightly.
...
Platform controllers (cert-manager, external-dns) use **Workload Identity**—federated managed identities, no long-lived secrets in manifests. Application secrets are intended for **Key Vault** and retrieved at runtime.
...
The cluster runs **kube-prometheus-stack**: Prometheus scrapes metrics, Grafana dashboards visualize health, and Alertmanager routes alerts (ingress 5xx bursts, crash loops, certificate expiry) once you point it at your notification channel.
...
The interesting part of this project isn't "Kubernetes on Azure"—it's tying together **immutable promotion**, **GitOps**, and **environment isolation** in a way you can explain on a whiteboard and then run for real.

If that matches what you're trying to learn, clone the repo, skim `ARCHITECTURE.md` and `DEPLOYMENT.md`, and watch for Part 2 next week—I'll walk through Terraform, cluster bootstrap, and the first deploy.
