...
I wanted a project that felt like real platform engineering—not a single `docker run`, not a slide deck of boxes, and not a cluster where “production” means changing a tag by hand. So I built an end-to-end path for [Google’s Online Boutique](https://github.com/GoogleCloudPlatform/microservices-demo) on Azure: infrastructure as code, CI that builds images once, GitOps that deploys by immutable digest, and guardrails that still let you run dev, stage, and prod on one AKS cluster without pretending they’re the same environment.
...
An earlier design used one registry and promoted by retagging. That’s simple on paper and messy in practice: tags are mutable, audit trails get fuzzy, and it’s too easy for prod to point at an image that was never validated in dev.
...
The remaining Online Boutique services (checkout, payment, email, shipping, recommendations, load generator) can be wired later from **upstream Google images** without this repo rebuilding them—useful when you want the full user journey, optional when you’re proving the platform.
...
Pipelines build from `apps/<service>/Dockerfile`, run Trivy (fail on high/critical findings), push to the dev registry, and open a GitHub PR that updates `image.digest` in the dev values file. Triggers are off by default—you run pipelines manually while learning, then enable branch triggers when you’re ready.
