...
**Tags (pick 5):** GitOps, Azure, Kubernetes, Argo CD, Platform Engineering  
**Repo link (use in story):** https://github.com/btilki/micser-apps-Azure-Terraform-Helm-GitOps-monitoring  
**Medium URL (Part 3):** https://medium.com/@btilki/promote-by-digest-gate-production-and-operate-gitops-release-day-on-azure-db5f20a5aafc  
**Medium URL (Part 2):** https://medium.com/@btilki/from-clone-to-https-deploying-online-boutique-on-azure-terraform-aks-and-your-first-pipeline-8b7303317cfb  
...
*Part 3 of 3. [Part 1](post-01-architecture-and-choices-corrected.md) — architecture. [Part 2](post-02-deploy-from-zero-to-dev-https-corrected.md) — deploy through dev HTTPS.*
...
1. **RBAC pre-check** — fails if the service principal cannot read the source ACR, push to the target ACR, or read required resource groups.
...
Register `promote-to-stage.yml` in Azure DevOps like CI: point at the YAML path, run manually, set parameter **`service`** to `frontend` first, then the other v1 services: `cartservice`, `currencyservice`, `productcatalogservice`, `redis-cart`. Leave **`digest`** empty to read from the source GitOps file, or paste `sha256:…` to override.
...
Run `promote-to-prod.yml` with **`service`** set; complete the **`promote-prod`** approval when the pipeline pauses; review the GitHub PR (only prod values should change; digest must match what you tested in stage).
...
Verify with the [release-verification](../runbooks/release-verification.md) checklist: pods ready, ingress healthy, and no TLS errors.
...
Prod secrets belong in **Key Vault** via Secrets Store CSI—not in Git. Platform controllers use **Workload Identity** (cert-manager, external-dns). Pipelines use **`promotion-azure-connection`** (ARM service connection) for ACR import and push; **`GITHUB_TOKEN`** in the **`variable-group-for-microservices`** Library group opens GitOps PRs on GitHub.

Longer treatment: [`SECURITY.md`](../../SECURITY.md) and [`docs/gitops/prod-branch-protection.md`](../gitops/prod-branch-protection.md). Maintain [`docs/gitops/prod-known-good-digests.md`](../gitops/prod-known-good-digests.md) and update it after each successful prod release.

---

## Observability on release day

**kube-prometheus-stack** in `monitoring` scrapes ingress and workload metrics. Grafana dashboards (see [`docs/runbooks/grafana-dashboards.md`](../runbooks/grafana-dashboards.md)) help answer: "Is this a bad deploy or a platform issue?"

Alertmanager routes worth configuring before prod traffic:

- Ingress **5xx** bursts — `BoutiqueIngress5xxBurst` and `BoutiqueHighHttp5xxRatio`.
- Pod **crash loops** after a new digest — e.g. `KubePodCrashLooping` / `KubePodNotReady`.
- **Certificate expiry** — cert-manager metrics; DNS-01 failures often show up as TLS errors first.
...
**Preferred path:** Git revert or restore the previous **`image.digest`** in `gitops/envs/prod/values-<service>.yaml`, merge through prod branch protection (two approving reviews if that is your rule on `main`), then sync prod app(s) in Argo CD.

Avoid `kubectl set image` except in emergencies—the cluster must match Git or the next sync will fight you.

Runbook flow ([`docs/runbooks/prod-rollback.md`](../runbooks/prod-rollback.md)):

1. Note the time of the last prod values merge or manual Sync.
2. Compare the current digest to known-good ([`prod-known-good-digests.md`](../gitops/prod-known-good-digests.md), previous commit, or stage if stage was last good).
...
| Runbook | When |
|---------|------|
| [ingress-5xx-triage](../runbooks/ingress-5xx-triage.md) | HTTP errors at the edge |
| [certificate-renewal-expiry](../runbooks/certificate-renewal-expiry.md) | TLS / ACME issues |
| [failing-argocd-sync-prod](../runbooks/failing-argocd-sync-prod.md) | Prod app won't sync or stays unhealthy |

---

## Hardening after the first prod release

Phases 8–9 in the repo cover policy bundles, Trivy in CI, budgets, dashboard polish, and optional smoke steps in promote pipelines. You do not need everything on day one—but **NetworkPolicies**, **Pod Security Standards** (namespace labels in `gitops/platform/*/namespace.yaml`), and **cert-manager** monitoring with the certificate runbooks should follow soon after.
